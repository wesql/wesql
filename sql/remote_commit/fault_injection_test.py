...
results = []
for scenario in ("disabled", "unarmed", "crash", "pause", "invalid", "symlink", "fifo"):
    ...
    try:
        code = child.wait(timeout=3 if scenario == "fifo" else 10)
    except subprocess.TimeoutExpired:
        child.kill()
        child.wait()
        raise
    expected = 86 if scenario in {"crash", "invalid", "symlink", "fifo"} else 0
    ...
    if scenario in {"crash", "pause"}:
        ...
        (root / (point + ".arm")).write_text(scenario + "\n")
        assert subprocess.run([str(args.driver), point], env=env).returncode == 0
        assert json.loads((root / (point + ".hit")).read_text()) == hit
    else:
        assert not (root / (point + ".hit")).exists()
    results.append({"scenario": scenario, "exit": code})
(args.output / "RESULT.json").write_text(json.dumps(results, indent=2) + "\n")
print("PASS: release control disabled/unarmed/crash/pause/one-shot/invalid/symlink/fifo")
