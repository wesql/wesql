...
/*
  unicode_ci / unicode_520_ci are compiled in strings/ctype-uca.cc and are
  not in include/m_ctype.h. Same extern pattern as mysys/charset.cc on 8.0.46.
*/
...
/* Allowlist collation closest to unicode_ci for SmartEngine indexes. */
const CHARSET_INFO *const k_allowlist = &my_charset_utf8mb4_0900_ai_ci;

bool needs_collation_rewrite(const CHARSET_INFO *cs) {
  return cs == &my_charset_utf8mb4_unicode_ci ||
         cs == &my_charset_utf8mb4_unicode_520_ci;
}

const char *collation_label(const CHARSET_INFO *cs) {
  if (cs != nullptr && cs->m_coll_name != nullptr) return cs->m_coll_name;
  return "unknown";
}

void warn_collation_rewrite(THD *thd, const char *field_name,
                            const CHARSET_INFO *from) {
  const char *old_name = collation_label(from);
  push_warning_printf(thd, Sql_condition::SL_WARNING,
                      ER_WESQL_DDL_COLLATION_REWRITTEN,
                      ER_THD(thd, ER_WESQL_DDL_COLLATION_REWRITTEN), field_name,
                      old_name, k_allowlist->m_coll_name);
  sql_print_warning(
      "WeSQL DDL compat: remapped collation of indexed column '%s' from %s "
      "to %s. Comparison rules changed.",
      field_name, old_name, k_allowlist->m_coll_name);
}

void warn_fk_stripped(THD *thd, const char *name) {
...
bool is_string_index_type(const Create_field *field) {
...
bool column_is_in_non_fk_key(const Alter_info *alter_info,
...
void rewrite_indexed_collations(THD *thd, HA_CREATE_INFO *create_info,
                                Alter_info *alter_info) {
  if (alter_info == nullptr) return;

  /*
    Table-default unicode_ci is inherited by indexed columns that have no
  ...
  */
  bool inherited_on_index = false;
  List_iterator<Create_field> it(alter_info->create_list);
  Create_field *field;
  while ((field = it++)) {
    if (!is_string_index_type(field)) continue;
    if (!column_is_in_non_fk_key(alter_info, field->field_name)) continue;
    const CHARSET_INFO *cs = get_sql_field_charset(field, create_info);
    if (!needs_collation_rewrite(cs)) continue;
    if (field->charset == nullptr ||
        field->charset == create_info->default_table_charset)
      inherited_on_index = true;
    field->charset = k_allowlist;
    field->is_explicit_collation = true;
    warn_collation_rewrite(thd, field->field_name, cs);
  }
  if (inherited_on_index &&
      needs_collation_rewrite(create_info->default_table_charset)) {
    create_info->default_table_charset = k_allowlist;
  }
}
...
void strip_foreign_keys(THD *thd, Alter_info *alter_info) {
  ...
  for (Key_spec *key : alter_info->key_list) {
    ...
    /* Parser emits a generated KEYTYPE_MULTIPLE next to each FOREIGN KEY. */
  ...
  for (const char *name : fk_names) warn_fk_stripped(thd, name);
}
...
bool wesql_ddl_compat_rewrite_create(THD *thd, HA_CREATE_INFO *create_info,
  ...
  rewrite_indexed_collations(thd, create_info, alter_info);
  return false;
}
