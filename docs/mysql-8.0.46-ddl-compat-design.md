...
授权：工作帖 `#3105bd58`。唯一执行人：白小纯。
长期分支：`8.0`，当前头
`5f543b8027ebc45464b49df3c6b0b1c9cfe9eb35`（#91）。
`9.7` 仍是 `4e26b01e194c3b7617c3491b9bc1ac7f94e39716`。不合进 `9.7`。

工作线「8.0 升到 8.0.46」已经正式验收通过：`#3105bd58`
（代码合入 `#d25da9d0`，构建树启动/读写 `#fc8fb997`，
install4 + 安装树 `--version` `#3105bd58`）。
这次不重跑 unique-run，不开 objstore28，不 retry `16d21c27`。

不要把 GitHub PR #85（`weicao/orm-ddl-rewrite`）当成这一步交付，
不改那条 PR，不合那条 PR。不把 `9.7` 上的 `sql/wesql_ddl_compat.cc`
直接贴过来。

## 1. 要解决什么

基线已经是 MySQL **8.0.46** 的 WeSQL。SmartEngine 当默认引擎时，
常见 ORM 的默认 `CREATE TABLE` 仍会硬失败。2026-08-22 在
WeSQL 8.0.35 上测过 Prisma / Django / Rails / Laravel / Drizzle
官方默认建表：20 条语句，11 条硬失败，每个 ORM 至少失败 1 条。
证据：`wesql-compat/results/REPORT.md`。升到 8.0.46 没有改
SmartEngine 这条限制，问题还在。
...
接线写在 8.0.46 的解析结果上，按 8.0.46 的接口自己做。
`9.7` 上已经合过同类垫层，只当产品范围对照，不当源码来源。
...
1. 在 GitHub `wesql/wesql` 的 `8.0` 上落地垫层：SmartEngine 的
   collation 改写 + 外键剥离 + 警告。基线是 8.0.46。
2. 自己测：上面四类例子；五个 ORM 默认脚手架能建表；InnoDB 外键
   不变；重启后 `SHOW CREATE TABLE` 仍是改写后的样子。
   用新的数据目录，不复用第 1 步 `/tmp/wesql-8.0.46-upgrade-datadir`，
   不碰 9.7 数据目录，不覆盖第 1 步 KEEP 日志。
3. PR 合进 `8.0`，不合进 `9.7`。临时分支从 `8.0` 开，合完再删。
...
- 不改 KEEP 容器 `wesql-8.0-upgrade`、`wesql-task27-v11`
- 不改 FULLTEXT / GEOMETRY / 生成列
- 不把 8.0.35 数据目录原地升级写成闸门
- 不飞书重复

## 6. 完成怎么算

- 这篇设计合进 `8.0`
- 后续代码 PR 合进 `8.0`，头能指出提交号
- 本席自己的建表、警告、`SHOW CREATE TABLE`、写入、读取、重启
  有能查的输出摘要
- `mysqld --version` 仍含 8.0.46 和 WeSQL 0.1.0
- 没有往 `9.7` 合，没有把 #85 合进去，没有新的对象存储 unique-run 包

第 1 步「8.0 升到 8.0.46」仍以 `#3105bd58` 为准。
本线只收 8.0.46 上的 SmartEngine 建表兼容。
