...
public:
  ...
  // @return value SeTableDef
  std::shared_ptr<SeTableDef> find(const std::string &table_name,
                                   bool* from_dict,
                                   bool lock = true);
  std::shared_ptr<SeTableDef> find(const char *table_name,
                                   int64_t name_len,
                                   bool* from_dict,
                                   bool lock = true);
  std::shared_ptr<SeTableDef> find(const dd::Table* dd_table,
                                   ...
                                   bool* from_dict,
                                   bool lock = true);
  std::shared_ptr<SeTableDef> find(THD* thd,
                                   const std::string& table_name,
                                   bool* from_dict,
                                   bool lock = true);
  int alloc_index_id(uint64_t &index_id);
  bool get_table_id(uint64_t &table_id);
  void add_uncommitted_keydefs(const std::unordered_set<std::shared_ptr<SeKeyDef>> &indexes);
  void remove_uncommitted_keydefs(const std::unordered_set<std::shared_ptr<SeKeyDef>> &indexes);
...
private:
  ...
  SeKeyDef* restore_index_from_dd(const dd::Properties& prop,
                                  ...
                                  uint keyno,
                                  int index_type);
  SeTableDef* restore_table_from_dd(const dd::Table* dd_table, const std::string& table_name);
  ...
  // lock[in] if true(by default), a read lock on table cache will be acquired.
  // @return value SeTableDef
  std::shared_ptr<SeTableDef> find_tbl_from_cache(const std::string &table_name, bool lock = true);
  const std::shared_ptr<SeKeyDef> &find(GL_INDEX_ID gl_index_id);
  // TODO(Zhao Dongsheng): this function is deprecated, remove it in future.
...
private:
  ...
  std::map<GL_INDEX_ID, std::shared_ptr<SeKeyDef>> m_index_num_to_uncommitted_keydef;
